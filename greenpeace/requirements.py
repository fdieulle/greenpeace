...
def __write_requirement_line(package: str, requirement: Dict[str, str]) -> None:
    if SPECIFIER in requirement:
        ...
        # Package with specifier
            ...
            return f'{package} {requirement[SPECIFIER]} {requirement[VERSION]}' # Keep exception if version is not part of the dictionary
    # Package without specifier or a particular file
    return package


def write_requirements_lines(packages: Union[List[str], Dict[str, str], Dict[str, Dict[str, str]]]) -> List[str]:
    ...
    raise TypeError(f'Only List[str], Dict[str, str], Dict[str, Dict[str, str]] ar supported but you give a {type(packages)}') 
...
def write_requirements(file_path: str, packages: Union[List[str], Dict[str, str], Dict[str, Dict[str, str]]]) -> None:
